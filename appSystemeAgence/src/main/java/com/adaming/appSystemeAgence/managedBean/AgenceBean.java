	...
	public void addClasseStandard(ActionEvent event) {
		...
		if(true) {
			...
			if (getClasseStandardService().addClasseStandard(classeStandard)){
				...
				message = new FacesMessage(FacesMessage.SEVERITY_INFO, "Succ̬s" ,"classeStandard added with success to DB");
			} else {
			...
			validForm = false;
			message = new FacesMessage(FacesMessage.SEVERITY_WARN, "Erreur", "Veuillez v̩rifier les champs");
		}
